"""
CageStatsApp: the Textual TUI application.
...
``CageStatsApp``
    ...
    visible panel by calling the rendering functions in ``cage_stats.ui.render``.
...
from cage_stats.config.config import Config
from cage_stats.fleet.fleet import Fleet, InstanceRuntime
from cage_stats.fleet.resolve import derive_name
...
from cage_stats.providers.logsource import LogTailer
from cage_stats.providers.mock import MockProvider, MockVllmProvider, mock_gpu_snapshot
from cage_stats.providers.proxy import TeeProxy, aiohttp_available, parse_proxy_addr
...
from cage_stats.ui.widgets import Panel


class CageStatsApp(App):
    CSS = """
            ...
            if not aiohttp_available():
                rt0.tee.push(
                    TeeEvent(
                        ts=time.time(),
                        kind="note",
                        text="proxy needs aiohttp — pip install 'cage_stats[proxy]'",
                    )
                )
                self._proxy = None
                self._proxy_desc = ""
